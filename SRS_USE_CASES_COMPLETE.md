...
### **NHÓM 8: QUẢN TRỊ HỆ THỐNG (SYSTEM ADMINISTRATION)**
...
| **Quy tắc nghiệp vụ** | - Người dùng chỉ xem được thửa đất được phép theo quyền hạn<br/>- Kết quả tìm kiếm được giới hạn 100 bản ghi<br/>- Hỗ trợ tìm kiếm theo từ khóa và bộ lọc nâng cao<br/>- Org3 chỉ xem được thửa đất thuộc quyền sử dụng |
...
| **Thuộc tính** | **Mô tả** |
|----------------|-----------|
| **Tên chức năng** | Hiển thị tất cả thửa đất của một người sử dụng đất |
| **Tác nhân** | Toàn bộ người dùng đã đăng nhập |
...
| **Quy trình thực hiện** | 1. Cán bộ truy cập trang quản lý thửa đất<br/>2. Hệ thống tải danh sách tất cả thửa đất<br/>3. Hệ thống hiển thị thông tin: ID thửa đất, vị trí, người sử dụng đất, diện tích, mục đích sử dụng, trạng thái pháp lý<br/>4. Hệ thống cung cấp bộ lọc: theo vị trí, mục đích sử dụng, trạng thái pháp lý<br/>5. Hệ thống cho phép sắp xếp theo diện tích, ngày tạo<br/>6. Hệ thống hiển thị thống kê tổng hợp<br/>7. Hệ thống hỗ trợ phân trang và xuất dữ liệu |
...
| **Quy tắc nghiệp vụ** | - Chỉ cán bộ Org1 và Org2 mới xem được toàn bộ<br/>- Hiển thị tối đa 50 thửa đất trên một trang<br/>- Thống kê được cập nhật theo thời gian thực<br/>- Dữ liệu được lấy trực tiếp từ blockchain |
...
| **Quy tắc nghiệp vụ** | - Thông tin được lấy trực tiếp từ blockchain<br/>- Org3 chỉ xem được thửa đất thuộc quyền sử dụng<br/>- Lịch sử hiển thị theo thứ tự thời gian mới nhất<br/>- Chỉ hiển thị tài liệu đã được xác minh<br/>- Thông tin giấy chứng nhận chỉ hiển thị khi có GCN |
...
| **Quy tắc nghiệp vụ** | - Lịch sử được lưu trữ bất biến trên blockchain<br/>- Org3 chỉ xem được lịch sử thửa đất thuộc quyền sử dụng<br/>- Chỉ hiển thị giao dịch được phép xem<br/>- Thông tin được xác thực từ nhiều node<br/>- Mỗi giao dịch đều có timestamp và người thực hiện |
...
| **Tác nhân** | Công dân (Org3) - người sử dụng đất hiện tại |
...
| **Tác nhân** | Công dân (Org3) - người sử dụng đất |
...
| **Thuộc tính** | **Mô tả** |
|----------------|-----------|
| **Tên chức năng** | Hiển thị giao dịch của một người sử dụng đất cụ thể |
| **Tác nhân** | Toàn bộ người dùng đã đăng nhập |
...
| **Tác nhân** | Admin tổ chức |
| **Mục đích** | Cấu hình các tham số hệ thống và tùy chỉnh hoạt động cho tổ chức |
| **Tiền điều kiện** | - Đã đăng nhập với quyền Admin tổ chức<br/>- Có quyền thay đổi cấu hình hệ thống của tổ chức<br/>- Hiểu rõ về các tham số cấu hình |
| **Kết quả đạt được** | - Cấu hình hệ thống của tổ chức được cập nhật thành công<br/>- Các thay đổi có hiệu lực ngay lập tức hoặc sau khi restart<br/>- Lịch sử thay đổi được ghi nhận<br/>- Hệ thống hoạt động theo cấu hình mới của tổ chức |
| **Quy trình thực hiện** | 1. Admin tổ chức truy cập trang cài đặt hệ thống<br/>2. Admin chọn nhóm cài đặt: bảo mật, hiệu suất, giao diện, thông báo<br/>3. Admin chỉnh sửa các tham số cấu hình cho tổ chức<br/>4. Hệ thống validate tính hợp lệ của cấu hình<br/>5. Admin xác nhận áp dụng thay đổi<br/>6. Hệ thống backup cấu hình cũ của tổ chức<br/>7. Hệ thống áp dụng cấu hình mới cho tổ chức<br/>8. Hệ thống ghi nhận lịch sử thay đổi<br/>9. Hệ thống restart các service cần thiết của tổ chức |
| **Trường hợp ngoại lệ** | - Cấu hình không hợp lệ: Hệ thống từ chối và đưa ra hướng dẫn<br/>- Lỗi áp dụng cấu hình: Hệ thống rollback về cấu hình cũ<br/>- Mất quyền: Hệ thống đăng xuất và yêu cầu đăng nhập lại<br/>- Không đủ quyền: Hệ thống từ chối truy cập |
| **Quy tắc nghiệp vụ** | - Chỉ Admin tổ chức mới có quyền thay đổi cấu hình hệ thống của tổ chức đó<br/>- Mọi thay đổi phải được backup trước<br/>- Ghi nhận đầy đủ lịch sử thay đổi<br/>- Một số cài đặt cần restart hệ thống<br/>- Cấu hình của tổ chức không ảnh hưởng đến tổ chức khác |
...
| **Tác nhân** | Admin tổ chức |
| **Mục đích** | Theo dõi hoạt động hệ thống và troubleshooting cho tổ chức |
| **Tiền điều kiện** | - Đã đăng nhập với quyền Admin tổ chức<br/>- Có quyền truy cập logs hệ thống của tổ chức<br/>- Hệ thống đang ghi nhận logs |
| **Kết quả đạt được** | - Xem được nhật ký hoạt động của tổ chức theo thời gian<br/>- Tìm kiếm và lọc logs theo tiêu chí<br/>- Phân tích logs để phát hiện vấn đề<br/>- Xuất logs để phân tích offline<br/>- Theo dõi hoạt động người dùng trong tổ chức |
| **Quy trình thực hiện** | 1. Admin tổ chức truy cập trang quản lý logs<br/>2. Admin chọn loại log: application, system, security, blockchain, user activity<br/>3. Admin thiết lập bộ lọc: thời gian, mức độ, nguồn, người dùng<br/>4. Hệ thống tìm kiếm và hiển thị logs phù hợp với tổ chức<br/>5. Admin có thể xem chi tiết từng log entry<br/>6. Hệ thống cung cấp tính năng search và highlight<br/>7. Admin có thể xuất logs ra file<br/>8. Hệ thống cho phép thiết lập alerts cho logs quan trọng<br/>9. Admin có thể thiết lập retention policy cho logs |
| **Trường hợp ngoại lệ** | - Logs quá lớn: Hệ thống phân trang và giới hạn hiển thị<br/>- Không tìm thấy logs: Hệ thống thông báo "Không có logs phù hợp"<br/>- Lỗi đọc file log: Hệ thống thông báo lỗi và đề xuất khắc phục<br/>- Không đủ quyền: Hệ thống từ chối truy cập logs của tổ chức khác |
| **Quy tắc nghiệp vụ** | - Chỉ Admin tổ chức mới có quyền truy cập logs hệ thống của tổ chức đó<br/>- Logs được lưu trữ theo mức độ ưu tiên và tổ chức<br/>- Tự động xóa logs cũ theo chính sách retention của tổ chức<br/>- Logs quan trọng được backup định kỳ<br/>- Mã hóa logs nhạy cảm khi lưu trữ<br/>- Admin có thể thiết lập retention policy riêng cho tổ chức |

---

## 🎯 **KẾT LUẬN**

### **Tổng số chức năng đã hoàn thành: 64 Use Cases**

#### **Phân bổ chức năng theo nhóm:**
- **Nhóm 1: Xác thực** - 8 chức năng (UC-01 đến UC-08)
- **Nhóm 2: Quản lý người dùng** - 5 chức năng (UC-09 đến UC-13)  
- **Nhóm 3: Quản lý hồ sơ cá nhân** - 2 chức năng (UC-14 đến UC-15)
- **Nhóm 4: Quản lý thửa đất** - 7 chức năng (UC-16 đến UC-22)
- **Nhóm 5: Quản lý tài liệu** - 15 chức năng (UC-23 đến UC-38)
- **Nhóm 6: Quản lý giao dịch** - 19 chức năng (UC-39 đến UC-57)
- **Nhóm 7: Quản lý thông báo** - 5 chức năng (UC-58 đến UC-62)
- **Nhóm 8: Quản trị hệ thống** - 2 chức năng (UC-63 đến UC-64)

### **Các tác nhân chính:**
- **Admin Org1**: Quản trị viên Sở Tài nguyên & Môi trưxx`ờng (quản lý tài khoản, cấu hình hệ thống, logs)
- **Admin Org2**: Quản trị viên cấp xã (quản lý tài khoản, cấu hình hệ thống, logs)
- **Admin Org3**: Quản trị viên công dân (quản lý tài khoản, cấu hình hệ thống, logs)
- **Org1**: Sở Tài nguyên & Môi trường (quản lý thửa đất, phê duyệt giao dịch)
- **Org2**: Cán bộ hành chính cấp xã (xác minh tài liệu, xử lý giao dịch)
- **Org3**: Công dân (tạo yêu cầu, theo dõi giao dịch)
...
4. **Phân quyền theo tổ chức**: Admin tổ chức quản lý riêng từng tổ chức (tài khoản, cấu hình, logs)
...
8. **Quản trị phân tán**: Mỗi tổ chức có quyền quản trị riêng biệt
